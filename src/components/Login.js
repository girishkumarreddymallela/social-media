  ...
  const onSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      if (err.response) {
        ...
        setMessage('seems like Network error check Your internet connection or else  please try after some time'); // Handle other types of errors
      }
      setSubmitting(false);
    }
  };  
    ...
    <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={onSubmit}>
      {({ isSubmitting  }) => (
        <Form>
          ...
          <button type="submit" disabled={isSubmitting} className="J">Login</button>
        </Form>
      )}
    </Formik>
    <p className="K">Don't have an account? <Link to="/register" className="L">Sign Up</Link></p>
  </div>
  </div>
  );
};
